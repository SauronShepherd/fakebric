...
class Parser:
    ...
    def _advance(self)->Token:
        ...
        return token
    def _error(self,code:str,message:str,token:Token|None=None)->DaxError:
        target=token or self.current; return DaxError(DaxDiagnostic(code,message,target.line,target.column,target.value))
    def _node(self,node:Expression,token:Token)->Expression:
        self.nodes+=1
    ...
    def parse(self)->Expression:
        ...
        return expression
    def _expression(self,min_precedence:int,depth:int)->Expression:
        ...
        while self.current.kind=="OP" and self.current.value in _PRECEDENCE:
            ...
            self._advance(); next_min=precedence if operator.value=="^" else precedence+1; right=self._expression(next_min,depth+1); left=self._node(BinaryOp(operator.value,left,right),operator)
        return left
    def _prefix(self,depth:int)->Expression:
        token=self.current
